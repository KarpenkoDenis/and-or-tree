...
TreeNode::TreeNode(GraphWidget *graphWidget)
    : graph(graphWidget)
{
...
void TreeNode::addEdge(Edge *edge)
{
    edgeList << edge;
    edge->adjust();
}

QList<Edge *> TreeNode::edges() const
{
    return edgeList;
}

void TreeNode::calculateForces()
{
    ...
    foreach (QGraphicsItem *item, scene()->items()) {
        TreeNode *node = qgraphicsitem_cast<TreeNode *>(item);
        if (!node)
            continue;
...
void TreeNode::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    graph-> handleNodeClick(this->node);
}

void TreeNode::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
    update();
